...
def parse_opt(known=False):
    ...
    parser.add_argument('--weights', type=str, default=r"OCR_Model\weights\best.pt", help='initial weights path')
    parser.add_argument('--cfg', type=str, default='', help='model.yaml path')
    parser.add_argument('--data', type=str, default='Dataset/OCR/data.yaml', help='dataset.yaml path')
...
def process_data():
    ...
    #     shutil.rmtree("runs")
    # #     os.makedirs("dataset")
    # shutil.move('../data collect/dataset',"dataset")
    # get the paths of all the images available
    img_paths = []
        
    for dirname, _, filenames in os.walk('Dataset/OCR/images'):
        for filename in filenames:
            img_paths.append(os.path.join('Dataset/OCR/images/', filename)) # local machine
    ...
    # write train.txt
    with open("Dataset/OCR/train.txt", 'w') as f:
        lines = list('\n'.join(train))
        f.writelines(lines)
    f.close

    # write validate.txt
    with open('Dataset/OCR/validate.txt', 'w') as f:
        lines = list('\n'.join(validate))
        f.writelines(lines)
    f.close

    # # write test.txt
    # with open('Talha/test.txt', 'w') as f:
    #     lines = list('\n'.join(test))
    #     f.writelines(lines)
    # f.close
    ...
    f=open(r"Dataset\OCR\labels\classes.txt",'r')
    names=f.read().split('\n')
    f.close()
    # print(names)
    f=open('Dataset/OCR/data.yaml','w')
    f.write("train: Dataset/OCR/train.txt/")
    f.write("\n")
    f.write("val: Dataset/OCR/validate.txt/")
    f.write("\n")
    # f.write("test: Talha/test.txt/")
    # f.write("\n")
